...
L'operazione di permutazione è molto importante in quanto diffonde la dipendenza dei bit in/out. Nel caso di DES però le permutazioni finale e iniziale non hanno effetto. Supponiamo di non averle. Se assumiamo per assurdo che il cifrario cosi ottenuto sia facile da violare, vogliamo dimostrare che anche DES È FACILE DA VIOLARE. Chiamiamo il cifrario senza permutazione EDS. Se posso violare EDS, basta permutare ciò che ottengo e violo DES.

Quelle che hanno veramente effetto sono quelle implementato all'interno di ogni round. 

Come sono state definite quindi le permutazioni iniziale e finale? E' di fatto una permutazione regolare (vedi immagine).