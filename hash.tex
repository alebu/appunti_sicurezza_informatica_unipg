...
\item $\frac{coppie \: di \: hash \: uguali}{possibili \: coppie} = \frac{k}{k^2} = \frac{1}{k}$ : probabilità che una coppia di messaggi collida
\end{itemize}
...
\end{equation}
Assunto per ipotesi che gli eventi considerati siano mutualmente esclusivi, ovvero:
\begin{equation}
...
Poiché:
\begin{equation}
P_{i^{*}} = \frac{numero \: di \: hash \: diversi \: da \: h(m_i)}{numero \: di \: hash} = \frac{k - (i - 1)}{k} \forall i = 1,2,...,N
\end{equation}
Allora:
\begin{equation}
...
Vale tuttavia la pena riflettere sul perché dell'importanza della resistenza di una gunzione hash alle collisioni. L'importanza di tale proprietà è infatti meno scontata di quella della resistenza alla preimmagine.
...
L'attacco, per come è presentato, sembra essere un attacco alla pre-immagine (Bob deve calcolare un messaggio $m'_{A}$ avente un dato hash), non un attacco alle collisioni. Se si implementa seguendo questa filosofia, l'attacco è altamente inefficiente. Infatti, per calcolare $m'_{A}$, Bob deve:
...
\item posto $(i - 1) = \mid L_m \mid = \mid L_h \mid \: con \: i > 1$, generare l'i-esimo messaggio $m_i$ nel seguente modo:
	\begin{itemize}
...
\Pr \{ h(m_i) = h(m_j) \mid m_i \: e \: m_j  \mbox{ hanno significato diverso} \} \approx
\end{equation}
\begin{equation} \label{eq:pr_coll_2}
\approx \Pr \{ m_i = m_j \mid m_i \: e \: m_j  \mbox{ hanno significato diverso} \} \cdot \Pr \{ h(m_i) = h(m_j) \} = 
\end{equation}
...
Si è visto che, in linea di principio, un algoritmo di digest può sostituire un algoritmo di cifratura/decifratura a chiave segreta in tutte le sua applicazioni. Naturalmente vale anche il viceversa, cioè, disponendo di un algoritmo di cifratura a chiave segreta, è possibile ottenere un algoritmo di digest.
