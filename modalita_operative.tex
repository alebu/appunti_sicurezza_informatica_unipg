...
\end{figure}
\subsection{EDE con CBC Outside}
...
\subsection{CBC Outside vs Inside (Opzionale)}
...
\subsubsection{Cifratura tripla con solo due chiavi}
...
L'obiettivo è quello di stimare il numero $cc_i$ di coppie candidate $<K_i, K_j>$ dopo il test i-esimo. Definendo $c_{tot}$ come il numero delle coppie totali iniziali, tale valore è dato dalla seguente espressione:
\[
...
\]
Superare il test significa trovare una coppia $<K_A, K_B>$ tale che $E(K_{A}, m_{1}) = r_A = r_B = D(K_{B}, c_{1})$. Tale probabilità è equivalente alla probabilità di prendere una coppia di stringhe random  di 64 bit $<r_A, r_B>$ e constatare che $r_A = r_B$. La probabilità che tale evento si verifichi è pari a:
\[
